...
parser.add_argument(
    "--debug", "-d", action="count", default=0, help="Debug information (on stderr)"
)
...
parser.add_argument(
    "--extra-ansible-opts",
    dest="extra_ansible_opts",
    type=str,
    nargs="*",
    required=False,
    help="Any extra arguments to the `ansible-playbook` command",
)
...
parser.add_argument(
    ...
    type=str,
    required=False,
    default="/tmp/clushible",
    help="Which directory to put the log file in. (uses mktemp)"
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Do a dry run or not. A dry run will show commands from the runners, but no Ansible will be executed.",
)
